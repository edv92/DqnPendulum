...
from dqn_agent_from_bottom import Q_Agent
from dqn_agent_from_bottom import NeuralNetworkz
import pygame
import sys
import numpy as np
import torch
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("DQN results")
    ...
    deepQModel = NeuralNetworkz(num_obs, num_actions, num_neurons).to(device)
    deepQModel.load_state_dict(torch.load("models14/103898.pth"))
    deepQModel.eval()
    ...
    while running:
        ...
        action = torch.tensor([[torch.argmax(deepQModel(state_tensor))]], device=device)
        #print(action)
        ...
        """
        if states_k[2] >= 0:
            states_k[2] += np.random.uniform(0,0.15)
        else:
            states_k[2] += np.random.uniform(-0.15, 0)
        """
